...
class GHTorrent(object):
    ...
    def getIssues(self, owner, repo):
        ...
        reposql2 = s.sql.text('SELECT issues.issue_id, max(issue_events.created_at) as most_recent FROM issues, issue_events WHERE issues.repo_id = :repoid and issue_events.action != "closed" and issues.issue_id = issue_events.issue_id group by issues.issue_id ORDER BY issues.created_at DESC')
